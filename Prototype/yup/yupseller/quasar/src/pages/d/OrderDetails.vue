      ...
      >
        ...
        <template v-else>
            ...
            <div class="col-8">
                ...
                <h2 class="q-my-xs font35 font_600">Order #{{this.$route.params.orderno}} <q-chip
                    square
                    class="ls1 font12 text-uppercase"
                >{{this.orderdata.order_status}} </q-chip>
                </h2>
                <span
                class="lsp3"
                style="color: #7a92a5;"
                >Placed on {{this.odate}} {{this.omonth}}, {{this.oyear}} @ {{this.otime}} PM</span>
            </div>
            <q-space />
            <div class="col">
                <q-btn
...
export default {
  ...
  isLoggedIn () {
    ...
    mounted () {
      this.isloading = true
    this.$store.dispatch('example/fetchorderinfo', {
    ...
    .then((response) => {
      ...
      this.odate = date.getDate()
      this.oyear = date.getUTCFullYear()
      this.otime = date.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', second: 'numeric', hour12: false })
      // this.omonth = date.getMonth()
      this.omonth = date.toLocaleString('default', { month: 'long' }) 
